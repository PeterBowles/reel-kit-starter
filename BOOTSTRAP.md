...
2. In the current folder run: git clone https://github.com/PeterBowles/reel-kit-starter . (this folder must be empty). Then clone https://github.com/browser-use/video-use into ~/Developer/video-use (skip if present). Record the video-use path in CLAUDE.md where it says <VIDEO_USE>.
...
5. Brand interview. First ask: "Do you have a brand guide, logo, style sheet, or a website with your colours? Put any files in brand/ (PDF, PNG, HTML, anything) or give me the URL." If they do, read those files or the site and extract primary, accent and background colours, the font, and the logo; show what you found and ask them to confirm. Only ask for what you could not find. Otherwise ask one question at a time: company name; primary colour, accent colour, background colour (hex, or describe and propose); logo file (optional, copy to brand/logo.png); caption style, show me the three presets from brand/README.md and ask which; text case (sentence/upper). Write brand/brand.json from brand/brand.template.json.
